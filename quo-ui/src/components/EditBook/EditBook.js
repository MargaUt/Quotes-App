...
class EditBook extends Component {
  ...
  handleBookPages = (event) => this.setState({ booksPages: event.target.value});
        ...
        <div className="card-body">
         <form onSubmit={this.handleSubmit}>
            <div className="form-group">
              <div className="form-group">
                <label  for="inlineFormInputAuthor">Book author ({this.state.author}):</label>
                <input type="text" className="form-control" id="inlineFormInputAuthor" value={this.state.author}
                    onChange={this.handleBookAuthor} placeholder={this.state.author}/>
              </div>


                <div className="form-check">
                  <input className="form-check-input" type="checkbox" checked={this.state.bookIsFinished}
                    onChange={this.handleFinishedToReadBook} id="autoSizingCheck2"/>
                  <label className="form-check-label" for="autoSizingCheck2"> Book is
                  {this.state.bookIsFinished ? "finished to read" : " not finished to read"} 
                  </label>
                </div>

                
              <div className="form-group">
                <label  for="inlineFormInputPages"> Book pages ({this.state.booksPages}):</label>
                <input type="number"
                    value={this.state.booksPages}
                    onChange={this.handleBookPages} className="form-control" id="inlineFormInputPages" placeholder={this.state.booksPages}/>
              </div>

              <div className="form-group">
                <label for="inlineFormInputYear">  Book released Year ({this.state.releasedYear}):</label>
                <input type="number"
                    value={this.state.releasedYear}
                    onChange={this.handleReleasedYear} className="form-control" id="inlineFormInputYear" placeholder={this.state.releasedYear}/>
              </div>
              
              <div className="form-group">
                <label  for="inlineFormInputTitle">Book title ({this.state.title}):</label>
